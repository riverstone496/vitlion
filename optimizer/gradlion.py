...
class GradLion(Optimizer):
    ...
    def step(self, closure: Optional[Callable] = None):
        ...
        for p, exp_avg in zip(params, exp_avgs):
            ...
            with torch.no_grad():
                dm += torch.norm(exp_avg - exp_avg_prev)**2
...
# # update functions
# def update_fn_grad(p, grad, exp_avg, lr, wd, beta1, beta2):
#     # stepweight decay
#     p.data.mul_(1 - lr * wd)

#     if torch.distributed.is_initialized():
#         grad = grad.to(torch.int8)
...
#                 self.elapsed_time += update_fn_grad(p, grad, exp_avg, lr, wd, beta1, beta2)
#         return loss
